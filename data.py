...
def get_imagenette(datapath, split, batch_size, shuffle, transform=TRANSFORM):
    ds = torchvision.datasets.Imagenette(root=datapath, split=split, transform=transform, download=False)
    loader = torch.utils.data.DataLoader(ds, shuffle=shuffle, batch_size=batch_size)
    return ds, loader